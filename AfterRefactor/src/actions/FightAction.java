...
public class FightAction implements BattleAction {
    ...
    public boolean execute(Player player, Enemy enemy, GameHandler handler) {
        ...
        if (!playerPovemon.getIsAlive() && player.leftAlive()) {return new SwapAction().execute(player, enemy, handler);}
        if (!enemyPovemon.getIsAlive()) {Povemon nextPovemon = enemy.switchNextPovemon();
            if (nextPovemon != null) {
